...
turn = 1*deg2rad(-turn_lim:turn_lim);
...
%     B.del_shi = L_R*turn; % turn angles range for B
    ...
    A.R = R(A.V,A.phi); % Turn radius for A
    B.R = R(B.V,B.phi); % Turn radius for B
elseif strcmp(con,'B')
...
man.shi = @(shi0,del_shi) shi0 + del_shi; % heading range post turn for A
man.p_A = @(del_shi) [1-cos(del_shi);sin(del_shi)] * A.R .* sign(del_shi);
man.p_B = @(del_shi) B.p0 + [(cos(B.shi0) - cos(B.shi0+del_shi));(-sin(B.shi0) + sin(B.shi0+del_shi))] * B.R .* sign(del_shi);

% Straight Line position
str.shi = @(shi0) ones(1,length(turn)) * shi0; % heading post turn
str.p_A = @(t) [zeros(1,length(turn)); t * A.V]; % B maneuvering
str.p_B = @(t) B.p0 + [sin(B.shi0);cos(B.shi0)] * t .* B.V; % A maneuvering

switch con
    case {'AB+','AB-'}
        A.shi = man.shi(A.shi0,A.del_shi);
        B.shi = man.shi(B.shi0,B.del_shi);
        t = time(A.V,A.del_shi,A.phi); % time elasped at the end of turn considering A
        A.p = man.p_A(A.del_shi);
        B.p = man.p_B(B.del_shi);
        A.p_orig = str.p_A(t); B.p_orig = str.p_B(t);

    case 'B'
        A.shi = str.shi(A.shi0);
        B.shi = man.shi(B.shi0,B.del_shi);
        t = time(B.V,B.del_shi,B.phi); % time elasped at the end of turn considering B
        A.p = str.p_A(t);
        B.p = man.p_B(B.del_shi);
        A.p_orig = str.p_A(t); B.p_orig = str.p_B(t);        
...
V_Ry = V_y(B.V,B.shi) - V_y(A.V,A.shi); % x coordinate of V_R
V_R = [V_Rx; V_Ry]; % relative velocity vector between A and B in A's frame
for i = 1:length(turn)
...
state_APi_orig = A2i(A.p,A.shi,A); A.pi_orig = state_APi_orig.P; % position of A in inertial frame
state_BPi_orig = A2i(B.p,B.shi,A); B.pi_orig = state_BPi_orig.P; % position of B in inertial frame
...
pos_x = min(rad2deg(turn)); pos_y = D_AB(find(rad2deg(turn) == pos_x));
...
pos_x = max(rad2deg(turn)); pos_y = d_smin(find(rad2deg(turn) == pos_x));
...
if bank(j) == 30 || length(bank) == 1
    ...
    M3 = "Time to Min Sep";
    pos_x = min(rad2deg(turn)); pos_y = t_tot(find(rad2deg(turn) == pos_x));
    text(pos_x-10, pos_y, num2str(bank(j)));
